...
Pointer::Pointer(Canvas* canvas) : canvas(canvas), selectedPoint(nullptr), selectedFigure(nullptr), lastMousePos(QPoint(0,0)) {}
    ...
    for (Figure* fig : canvas->getFigures()) {
        ...
        for (const QPoint& vertex : fig->getVertices()) {
            if (QLineF(pos, vertex).length() < fig->radius) {
                ...
                emit canvas->figureSelected(fig->getBorderColor(), fig->getFillColor(), fig->getThickness());
...
bool Pointer::onMouseMove(QMouseEvent* event) {
    ...
    if (selectedVertexIndex >= 0) {
        ...
        if (Rectangle* rect = dynamic_cast<Rectangle*>(selectedFigure)) {
            // Get vertices in correct order: top-left, top-right, bottom-right, bottom-left
            const auto vertices = rect->getVertices();

            // Apply edge-specific movement constraints
            rect->moveEdge(selectedEdgeIndex, delta);
        } else {
            // Original behavior for other figures
